...
const Game = {
	...
	update: function() {if (window.cheated==true) {document.getElementById("html").style.backgroundColor = "red";document.getElementById("greenbar").style.backgroundColor = "black";document.getElementById("house").src="./imgs/madhouse.png"};window.multi=parseInt(window.multi);window.cash = Math.floor(window.cash);txt.innerText = "Your cash is currently: "+window.cash;btn.innerHTML = `<span style="color: green;">Get +${window.multi} Cash</span>`;tmn.innerHTML = `<span style="color: orange;">Time spent playing: <br>${window.time} seconds.</span>`;ttl.innerText = `Cash: ${window.cash}, Multi: ${window.multi}, Time: ${window.time}.`},
	LocLoad: function(){
		var save;
